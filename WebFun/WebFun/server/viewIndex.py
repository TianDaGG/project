...
def GetWorkRoot():
    ...
    workDir = os.getcwd() #获取当前工作目录路径
    workDir += "\\server\\templates\\postMessage"
    if workDir[0] != "E":
        workDir = "C:\\workroot\\WebFun\\WebFun\\server\\templates\\postMessage"

    if "online" in session and  session["online"] != "":
        workDir = os.getcwd() #获取当前工作目录路径
        workDir += "\\server\\templates\\postPrivate"
        if workDir[0] != "E":
            workDir = "C:\\workroot\\WebFun\\WebFun\\server\\templates\\postPrivate"
...
@indexBp.route("/test",methods=["POST"])
def GetEditFile():
    '''post测试'''
    ip = request.remote_addr
    method = request.method
    return "["+ip+method+"]吴工，测试成功！"
